...
void insert_x_node(struct cube *cube, short w_index, short x_index)
{
	...
	w_node->x_axis[x_index]->y_floor = (int *) malloc(BSC_Y_MAX * sizeof(int));
	w_node->x_axis[x_index]->y_axis = (struct y_node **) malloc(BSC_Y_MAX * sizeof(struct y_node *));
}
...
void insert_y_node(struct cube *cube, short w_index, short x_index, short y_index)
{
	...
	x_node->y_axis[y_index]->z_axis = (struct z_node *) malloc(BSC_Z_MAX * sizeof(struct z_node));
}
...
void insert_z_node(struct cube *cube, short w_index, short x_index, short y_index, short z_index, int key, void *val)
{
	...
	if (cube->volume == 0)
	{
		...
		x_node->y_floor = (int *) malloc(BSC_Y_MAX * sizeof(int));
		x_node->y_axis = (struct y_node **) malloc(BSC_Y_MAX * sizeof(struct y_node *));

		y_node = x_node->y_axis[0] = (struct y_node *) calloc(1, sizeof(struct y_node));

		y_node->z_axis = (struct z_node *) malloc(BSC_Z_MAX * sizeof(struct z_node));
	...
	if (y_node->z_size >= BSC_Z_MAX)
	{
		split_y_node(cube, w_index, x_index, y_index);

		if (x_node->y_size >= BSC_Y_MAX)
		{
			split_x_node(cube, w_index, x_index);

			if (w_node->x_size >= BSC_X_MAX)
			{
	...
	if (x_node->volume + BSC_Z_MIN < x_node->y_size * BSC_Z_MIN)
	{
		deflate_x_axis(cube, w_index, x_index);

		if (w_node->volume + BSC_Z_MIN * BSC_Y_MIN < w_node->x_size * BSC_Z_MIN * BSC_Y_MIN)
		{
			deflate_w_axis(cube, w_index);

			if (cube->volume + BSC_Z_MIN * BSC_Y_MIN * BSC_X_MIN < cube->w_size * BSC_Z_MIN * BSC_Y_MIN * BSC_X_MIN)
			{
				deflate_cube(cube);
			}
		}
...
void deflate_x_axis(struct cube *cube, short w_index, short x_index)
{
	...
	{
		if (x_node->y_axis[y_index]->z_size + x_node->y_axis[y_index + 1]->z_size < BSC_Z_MIN * 2)
		{
...
int main(int argc, char **argv)
{
	...
	printf("Time to insert %d elements: %f seconds. (random order)\n", max, (end - start) / 1000000.0);
	destroy_cube(cube);
